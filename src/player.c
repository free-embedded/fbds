...
void noTone() {
    ...
    upConfig.timerPeriod = 10000 * ADAPT_TO_48MHZ;
    Timer_A_configureUpMode(TIMER_A0_BASE, &upConfig);

    /* Initialize compare registers to generate PWM */
    compareConfig_PWM.compareValue = 20000 * ADAPT_TO_48MHZ;
...
void tone(int frequencyHz) {
    /* Configuring Timer_A0 for Up Mode and starting */
    upConfig.timerPeriod = 250000 / frequencyHz * ADAPT_TO_48MHZ;
    Timer_A_configureUpMode(TIMER_A0_BASE, &upConfig);

    /* Initialize compare registers to generate PWM */
    compareConfig_PWM.compareValue = 125000 / frequencyHz * ADAPT_TO_48MHZ;
    Timer_A_initCompare(TIMER_A0_BASE, &compareConfig_PWM); // For P2.7
